...
.factory('Snackbar', ['$window', function($window) {
...
.filter('conversationFrom', function() {
  return function(p, loggedInUser) {
    ...
    for(var i = 0; i < p.length; i++) {
      if (p[i].username != loggedInUser.username)
        final = p[i];
    }
...
angular.module('trudesk.controllers.login', []).controller('LoginCtrl', function($http, $scope, $state, $localStorage, $ionicLoading, $ionicPopup) {
  // if (window.StatusBar) {
  //   // org.apache.cordova.statusbar required
...
angular.module('trudesk.controllers.dashboard', []).controller('DashCtrl', function($http, $scope, $state, $location, $ionicNavBarDelegate, $localStorage, Tickets) {
...
angular.module('trudesk.controllers.graphs', []).controller('GraphCtrl', function($scope, $http, _, Graphs) {
    $scope.renderGraphs = function() {
        Graphs.topGroups().then(function successCallback(response) {
            var arr = [];
...
angular.module('trudesk.controllers.ticketDetails', []).controller('TicketsDetailCtrl', function(
  ...
  Users.getAssignees().then(function successCallback(response) {
  ...
  //Right Nav Popover
  $scope.popover = $ionicPopover.fromTemplateUrl('templates/popover/popover-ticket-details.html', {
    scope: $scope
  }).then(function(popover) {
    $scope.popover = popover;
  });
  ...
  $scope.closeSetAssigneeModal = function() {
      if ($scope.setAssigneeModal !== undefined)
  ...
  $scope.addCommentFormSubmit = function() {
      ...
      Tickets.addComment($scope.ticket, comment).then(function successCallback(response) {
        //Comment Added
      }, function errorCallback(err) {
        console.log(err);
        $scope.showSnackbar(err, true);
