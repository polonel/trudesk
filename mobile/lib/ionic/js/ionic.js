...
*/
  ...
  function keyboardWaitForResize (callback, isOpening) {
    ...
    waitForResizeTimer = setInterval(function () {
      ...
      if (
        ++count < maxCount &&
        ((!isPortraitViewportHeight(viewportHeight) && !isLandscapeViewportHeight(viewportHeight)) ||
      ...
      s.setBreakpoint = function () {
        ...
        if (breakpoint && s.currentBreakpoint !== breakpoint) {
          var breakPointsParams =
            breakpoint in s.params.breakpoints ? s.params.breakpoints[breakpoint] : s.originalParams
      ...
      s.onTouchMove = function (e) {
        ...
        if (diff > 0 && currentTranslate > s.minTranslate()) {
          ...
          if (s.params.resistance)
            currentTranslate =
              s.minTranslate() - 1 + Math.pow(-s.minTranslate() + startTranslate + diff, s.params.resistanceRatio)
        } else if (diff < 0 && currentTranslate < s.maxTranslate()) {
          disableParentSwiper = false
          if (s.params.resistance)
            currentTranslate =
              s.maxTranslate() + 1 - Math.pow(s.maxTranslate() - startTranslate - diff, s.params.resistanceRatio)
        }
        ...
        if (s.params.freeMode) {
          ...
          if (s.params.freeModeMomentum) {
            ...
            if (newPosition < s.maxTranslate()) {
              ...
              if (
                Math.abs(s.snapGrid[nextSlide] - newPosition) < Math.abs(s.snapGrid[nextSlide - 1] - newPosition) ||
              ...
              if (s.params.loop && loadInDuplicate) {
                ...
                if (slide.hasClass(s.params.slideDuplicateClass)) {
                  var originalSlide = s.wrapper.children(
                    '[data-swiper-slide-index="' + slideOriginalIndex + '"]:not(.' + s.params.slideDuplicateClass + ')'
                  )
                  s.lazy.loadImageInSlide(originalSlide.index(), false)
                } else {
                  var duplicatedSlide = s.wrapper.children(
                    '.' + s.params.slideDuplicateClass + '[data-swiper-slide-index="' + slideOriginalIndex + '"]'
                  )
