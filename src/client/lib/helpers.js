...
helpers.init = function (reload) {
  ...
  this.UI.showLinkWarning('.link-warning')

  const layout = this.onWindowResize()
...
helpers.UI.showSnackbar_ = function (text, error) {
...
helpers.getTimezone = function () {
  ...
  // const settingSingleton = require('./singleton/settingsSingleton')
  // settingSingleton.init(err => {
  //   if (err) {
  //     console.error(err)
  //     return 'America/New_York'
  //   }
...
helpers.canUser = function (a, adminOverride) {
  ...
  if (!user) {
    console.log('Unable to get Session User')
    return false
  }

  const userRole = user.role
  if (!userRole || !a) return false

  const roleGrants = userRole.grants

  const hasAdmin = roleGrants.indexOf('admin:*') !== -1 || roleGrants.indexOf('*') !== -1
  if (adminOverride === true && hasAdmin) return true
  ...
  const result = _.filter(roleGrants, function (value) {
    if (value.startsWith(actionType + ':')) return value
  })

  if (_.isUndefined(result) || result.length < 1) return false
  if (result.length === 1) {
...
helpers.hasHierarchyEnabled = function (roleId) {
  ...
  if (!state.shared || !state.shared.roles) {
    console.error('Invalid Role State. [Has the state been loaded?]')
    return false
  }

  const roles = state.shared.roles.toJS()
...
helpers.parseRoleGrants = function (grants) {
  ...
  _.each(grants, function (grant) {
    ...
    obj[grantName] = {
      ...
      special: without(typePerm, '*', 'create', 'view', 'update', 'delete')
    }

    final = merge(final, obj)
  })
...
helpers.parseRoleHierarchy = function (roleId) {
  ...
  if (!state.shared || !state.shared.roleOrder) {
    console.error('Invalid Role State. [Has the state been loaded?]')
    return false
  }

  let roleOrder = state.shared.roleOrder.toJS()
  if (_.isUndefined(roleOrder)) return []
  roleOrder = roleOrder.order

  const idx = roleOrder.indexOf(roleId.toString())
  if (idx === -1) return []
...
helpers.getLoggedInRoleHierarchy = function () {
  ...
  if (!state.shared || !state.shared.sessionUser) {
    console.error('Invalid Session State. [Has the state been loaded?]')
    return false
  }

  return helpers.parseRoleHierarchy(state.shared.sessionUser.role._id)
}
...
helpers.hasHierarchyOverRole = function (roleIdToCheck) {
  ...
  const roleOrder = helpers.parseRoleHierarchy(state.shared.sessionUser.role._id)
  if (roleOrder.length < 1) return false

  const idx = roleOrder.indexOf(roleIdToCheck.toString())
...
helpers.hasPermOverRole = function (ownerRole, extRole, action, adminOverride) {
  ...
  if (!extRole) extRole = getSession().user && getSession().user.role
  if (!extRole) {
    console.error('Error: Invalid Role! Unable to get User role from state!')
    return false
  }

  if (action && !helpers.canUser(action, adminOverride)) return false
  if (!_.isObject(ownerRole) || !_.isObject(extRole)) {
    console.error('Invalid Role Sent to helpers.hasPermOverRole. [Must be role obj]')
  ...
  if (adminOverride === true) {
    if (extRole && extRole.isAdmin) {
      ...
      const r = state.shared.roles.toJS()

      const role = r.find(function (_role) {
...
helpers.UI.hierarchicalSlide = function (element) {
  ...
  if ($hierarchicalSlide.length) {
    $hierarchicalSlide.each(function () {
      ...
      const thisContext = $this.attr('data-slide-context') ? $this.closest($this.attr('data-slide-context'))[0] : 'window'
...
function get (path, obj, fallback) {
  const fb = fallback !== undefined ? fallback : '${'.concat(path, '}')
...
helpers.parseTemplateString = parseTpl
