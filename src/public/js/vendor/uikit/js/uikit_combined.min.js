!(function (core) {
      ...
      define('uikit', function () {
        ...
        return (
          (uikit.load = function (res, req, onload, config) {
            ...
            if (!base) throw new Error('Please define base path to UIkit in the requirejs config.')
  ...
  window && window.jQuery && core(window, window.jQuery, window.document)
})(function (global, $, doc) {
  'use strict'
  var transitionEnd,
    animationEnd,
    domReady,
    UI = {},
    _UI = global.UIkit ? Object.create(global.UIkit) : void 0
  if (
    ((UI.version = '2.25.0'),
    (UI.noConflict = function () {
      return _UI && ((global.UIkit = _UI), ($.UIkit = _UI), ($.fn.uk = _UI.fn)), UI
    }),
    ...
    (UI.$html = UI.$('html')),
    (UI.support = {}),
    (UI.support.transition = (transitionEnd = (function () {
      var name,
      ...
      for (name in transEndEventNames) if (void 0 !== element.style[name]) return transEndEventNames[name]
    })()) && { end: transitionEnd }),
    (UI.support.animation = (animationEnd = (function () {
      var name,
        element = doc.body || doc.documentElement,
        animEndEventNames = {
      ...
      for (name in animEndEventNames) if (void 0 !== element.style[name]) return animEndEventNames[name]
    })()) && { end: animationEnd }),
    (function () {
      Date.now =
        Date.now ||
    ...
    (UI.support.touch =
      ...
      (global.navigator.msPointerEnabled && 0 < global.navigator.msMaxTouchPoints) ||
      (global.navigator.pointerEnabled && 0 < global.navigator.maxTouchPoints) ||
      !1),
    ...
    (UI.Utils.removeCssRules = function (selectorRegEx) {
      ...
      selectorRegEx &&
        setTimeout(function () {
          try {
            for (_ref = document.styleSheets, _i = 0, _len = _ref.length; _i < _len; _i++) {
              for (
                stylesheet = _ref[_i],
                  idxs = [],
                ...
                stylesheet.cssRules[idx].type === CSSRule.STYLE_RULE &&
                  ...
                  idxs.unshift(idx)
              for (_k = 0, _len2 = idxs.length; _k < _len2; _k++) stylesheet.deleteRule(idxs[_k])
            }
          } catch (_error) {}
        }, 0)
    }),
    ...
    (UI.Utils.checkDisplay = function (context, initanimation) {
      ...
      return (
        context && !elements.length && (elements = $(context)),
        elements.trigger('display.uk.check'),
        initanimation &&
          ('string' != typeof initanimation && (initanimation = '[class*="uk-animation-"]'),
          elements.find(initanimation).each(function () {
            var ele = UI.$(this),
              anim = ele.attr('class').match(/uk\-animation\-(.+)/)
            ele.removeClass(anim[0]).width(), ele.addClass(anim[0])
          })),
        elements
      )
    }),
    (UI.Utils.options = function (string) {
      if ('string' != $.type(string)) return string
      ;-1 != string.indexOf(':') && '}' != string.trim().substr(-1) && (string = '{' + string + '}')
      ...
      return (
        (element = UI.$(element))
          ...
          .one(UI.support.animation.end, function () {
            element.removeClass(cls), d.resolve()
          }),
        element.css('display', ''),
        d.promise()
      )
    ...
    (UI.Utils.uid = function (prefix) {
      return (prefix || 'id') + new Date().getTime() + 'RAND' + Math.ceil(1e5 * Math.random())
    }),
    (UI.Utils.template = function (str, data) {
      for (
        ...
        if ((toc = tokens[i]).match(/\{\{\s*(.+?)\s*\}\}/))
          switch (
            ((cmd = (toc = tokens[(i += 1)])[0]), (prop = toc.substring(toc.match(/^(\^|\#|\!|\~|\:)/) ? 1 : 0)), cmd)
          ) {
            case '~':
              output.push('for(var $i=0;$i<' + prop + '.length;$i++) { var $item = ' + prop + '[$i];'), openblocks++
              break
            case ':':
              output.push('for(var $key in ' + prop + ') { var $val = ' + prop + '[$key];'), openblocks++
              break
            case '#':
              output.push('if(' + prop + ') {'), openblocks++
              break
            case '^':
              output.push('if(!' + prop + ') {'), openblocks++
              break
            case '/':
              output.push('}'), openblocks--
              break
            case '!':
        ...
        else output.push("__ret.push('" + toc.replace(/\'/g, "\\'") + "');")
        i += 1
      }
      return (
        (fn = new Function(
          ...
          [
            ...
            'try {',
            'with($data){',
            openblocks ? '__ret = ["Not all blocks are closed correctly."]' : output.join(''),
            '};',
    ...
    ((global.UIkit = UI).fn = function (command, options) {
      var args = arguments,
      ...
      return UI[component]
        ? this.each(function () {
            ...
            data || $this.data(component, (data = UI[component](this, method ? void 0 : options))),
              method && data[method].apply(data, Array.prototype.slice.call(args, 1))
          })
        : ($.error('UIkit component [' + component + '] does not exist.'), this)
    }),
    ($.UIkit = UI),
    ($.fn.uk = UI.fn),
    (UI.langdirection = 'rtl' == UI.$html.attr('dir') ? 'right' : 'left'),
    (UI.components = {}),
    (UI.component = function (name, def) {
      var fn = function (element, options) {
        ...
        return (
          ...
          (this.plugins = {}),
          this.element && this.element.data(name, this),
          this.init(),
          (this.options.plugins.length ? this.options.plugins : Object.keys(fn.plugins)).forEach(function (plugin) {
            fn.plugins[plugin].init && (fn.plugins[plugin].init($this), ($this.plugins[plugin] = !0))
          }),
          ...
          if (arguments.length)
            switch (arguments.length) {
              case 1:
                ...
                break
              case 2:
                ;(element = $(arguments[0])), (options = arguments[1])
            }
          return element && element.data(name) ? element.data(name) : new UI.components[name](element, options)
        }),
        UI.domready && UI.component.boot(name),
        fn
      )
    }),
    ...
    (UI.component.boot = function (name) {
      UI.components[name].prototype &&
        ...
        (UI.components[name].prototype.boot.apply(UI, []), (UI.components[name].booted = !0))
    }),
    (UI.component.bootComponents = function () {
      for (var component in UI.components) UI.component.boot(component)
    }),
    (UI.domObservers = []),
    (UI.domready = !1),
    (UI.ready = function (fn) {
      UI.domObservers.push(fn), UI.domready && fn(document)
    }),
    (UI.on = function (a1, a2, a3) {
      return a1 && -1 < a1.indexOf('ready.uk.dom') && UI.domready && a2.apply(UI.$doc), UI.$doc.on(a1, a2, a3)
    }),
    (UI.one = function (a1, a2, a3) {
      return a1 && -1 < a1.indexOf('ready.uk.dom') && UI.domready
        ? (a2.apply(UI.$doc), UI.$doc)
        : UI.$doc.one(a1, a2, a3)
    }),
            ...
            try {
              var observer = new UI.support.mutationobserver(
                UI.Utils.debounce(function (mutations) {
                  fn.apply(element, []), $element.trigger('changed.uk.dom')
                }, 50)
              )
              observer.observe(element, { childList: !0, subtree: !0 }), $element.data('observer', observer)
            } catch (e) {}
        }))
    }),
    ...
    UI.on('domready.uk.dom', function () {
      UI.init(), UI.domready && UI.Utils.checkDisplay()
    }),
    document.addEventListener(
      ...
      ((domReady = function () {
        ...
        ;(UI.$body = UI.$('body')),
          ...
          UI.on('changed.uk.dom', function (e) {
            UI.init(e.target), UI.Utils.checkDisplay(e.target)
          }),
          UI.trigger('beforeready.uk.dom'),
          UI.component.bootComponents(),
          requestAnimationFrame(
            ((memory = { dir: { x: 0, y: 0 }, x: window.pageXOffset, y: window.pageYOffset }),
            (fn = function () {
              var wpxo = window.pageXOffset,
                wpyo = window.pageYOffset
              ;(memory.x == wpxo && memory.y == wpyo) ||
                (wpxo != memory.x ? (memory.dir.x = wpxo > memory.x ? 1 : -1) : (memory.dir.x = 0),
                wpyo != memory.y ? (memory.dir.y = wpyo > memory.y ? 1 : -1) : (memory.dir.y = 0),
                (memory.x = wpxo),
                (memory.y = wpyo),
            ...
            UI.support.touch && UI.$html.on('touchmove touchend MSPointerMove MSPointerUp pointermove pointerup', fn),
            (memory.x || memory.y) && fn(),
            fn)
          ),
          ...
          UI.support.touch &&
            ...
            UI.$win.on(
              ...
              UI.Utils.debounce(
                (function () {
                  var fn = function () {
                    return $('.uk-height-viewport').css('height', window.innerHeight), fn
                  }
                  return fn()
                })(),
                100
      ...
      ('complete' != document.readyState && 'interactive' != document.readyState) || setTimeout(domReady),
      domReady)
    ),
    ...
    UI.$html
      .on('mouseenter touchstart MSPointerDown pointerdown', selector, function () {
        hoverset && $('.uk-hover').removeClass('uk-hover'), (hoverset = $(this).addClass('uk-hover'))
      })
      .on('mouseleave touchend MSPointerUp pointerup', function (e) {
        ;(exclude = $(e.target).parents(selector)), hoverset && hoverset.not(exclude).removeClass('uk-hover')
      })
  }
  return UI
}),
  (function ($) {
    if (!$.fn.swipeLeft) {
      ...
      $(function () {
        var now,
          ...
          deltaX = 0,
          deltaY = 0
        'MSGesture' in window && ((gesture = new MSGesture()).target = document.body),
          $(document)
            .on('MSGestureEnd gestureend', function (e) {
              var swipeDirectionFromVelocity =
                1 < e.originalEvent.velocityX
                  ? 'Right'
                  : e.originalEvent.velocityX < -1
                  ? 'Left'
                  : 1 < e.originalEvent.velocityY
                  ? 'Down'
            ...
            .on('touchstart MSPointerDown pointerdown', function (e) {
              ;('MSPointerDown' != e.type || isPrimaryTouch(e.originalEvent)) &&
                ((firstTouch = 'MSPointerDown' == e.type || 'pointerdown' == e.type ? e : e.originalEvent.touches[0]),
                (now = Date.now()),
                ...
                0 < delta && delta <= 250 && (touch.isDoubleTap = !0),
                (touch.last = now),
                (longTapTimeout = setTimeout(longTap, 750)),
                !gesture ||
                  ('MSPointerDown' != e.type && 'pointerdown' != e.type && 'touchstart' != e.type) ||
                  gesture.addPointer(e.originalEvent.pointerId))
            })
            .on('touchmove MSPointerMove pointermove', function (e) {
              ;('MSPointerMove' != e.type || isPrimaryTouch(e.originalEvent)) &&
                ((firstTouch = 'MSPointerMove' == e.type || 'pointermove' == e.type ? e : e.originalEvent.touches[0]),
                cancelLongTap(),
            ...
            .on('touchend MSPointerUp pointerup', function (e) {
              ;('MSPointerUp' != e.type || isPrimaryTouch(e.originalEvent)) &&
                (cancelLongTap(),
                (touch.x2 && 30 < Math.abs(touch.x1 - touch.x2)) || (touch.y2 && 30 < Math.abs(touch.y1 - touch.y2))
                  ...
                  : 'last' in touch &&
                    (isNaN(deltaX) || (deltaX < 30 && deltaY < 30)
                      ? (tapTimeout = setTimeout(function () {
                          var event = $.Event('tap')
                          ;(event.cancelTouch = cancelAll),
                            void 0 !== touch.el && touch.el.trigger(event),
                            touch.isDoubleTap
                              ? (void 0 !== touch.el && touch.el.trigger('doubleTap'), (touch = {}))
                              : (touchTimeout = setTimeout(function () {
                                  ;(touchTimeout = null),
                                    void 0 !== touch.el && touch.el.trigger('singleTap'),
                                    (touch = {})
                                }, 250))
                        }, 0))
                      : (touch = {}),
    ...
    function longTap () {
      ;(longTapTimeout = null), touch.last && (void 0 !== touch.el && touch.el.trigger('longTap'), (touch = {}))
    }
  ...
  (function (UI) {
    ...
    UI.component('stackMargin', {
      defaults: { cls: 'uk-margin-small-top', rowfirst: !1 },
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-margin]', context).each(function () {
            var ele = UI.$(this)
            ele.data('stackMargin') || UI.stackMargin(ele, UI.Utils.options(ele.attr('data-uk-margin')))
          })
        })
      },
      init: function () {
      ...
      process: function () {
        ...
        if ((UI.Utils.stackMargin(columns, this.options), !this.options.rowfirst)) return this
        ...
        return (
          pos_cache &&
            columns.each(function () {
              UI.$(this)[UI.$(this).position().left == pos_cache.left ? 'addClass' : 'removeClass'](
        ...
        boot: function () {
          UI.ready(function (context) {
            UI.$('iframe.uk-responsive-width, [data-uk-responsive]', context).each(function () {
              var ele = UI.$(this)
              ele.data('responsiveElement') || UI.responsiveElement(ele, {})
            })
          })
        },
        init: function () {
      ...
      (UI.Utils.stackMargin = function (elements, options) {
        ;((options = UI.$.extend({ cls: 'uk-margin-small-top' }, options)).cls = options.cls),
          (elements = UI.$(elements).removeClass(options.cls))
        var skip = !1,
          firstvisible = elements.filter(':visible:first'),
          offset = !!firstvisible.length && firstvisible.position().top + firstvisible.outerHeight() - 1
        !1 !== offset &&
          1 != elements.length &&
          elements.each(function () {
            ...
            column.is(':visible') &&
              (skip
                ...
                : column.position().top >= offset && (skip = column.addClass(options.cls)))
          })
      }),
      (UI.Utils.matchHeights = function (elements, options) {
        ;(elements = UI.$(elements).css('min-height', '')), (options = UI.$.extend({ row: !0 }, options))
          ...
          if (!(group.length < 2)) {
            ...
            group
              ...
              .each(function () {
                var element = UI.$(this),
                  height =
                    max - ('border-box' == element.css('box-sizing') ? 0 : element.outerHeight() - element.height())
          ...
          ? (elements.first().width(),
            setTimeout(function () {
              ...
              elements.each(function () {
                ...
                offset != lastoffset &&
                  group.length &&
                  (matchHeights(UI.$(group)), (group = []), (offset = ele.offset().top)),
                  group.push(ele),
                  (lastoffset = offset)
              }),
                group.length && matchHeights(UI.$(group))
            }, 0))
          : matchHeights(elements)
      }),
      (cacheSvgs = {}),
      (UI.Utils.inlineSvg = function (selector, root) {
        UI.$(selector || 'img[src$=".svg"]', root || document).each(function () {
          var img = UI.$(this),
            src = img.attr('src')
          if (!cacheSvgs[src]) {
            var d = UI.$.Deferred()
          ...
          cacheSvgs[src].then(function (svg) {
            ...
            img.attr('id') && $svg.attr('id', img.attr('id')),
              ...
              img.attr('width') &&
                ($svg.attr('width', img.attr('width')), img.attr('height') || $svg.removeAttr('height')),
              img.attr('height') &&
                ($svg.attr('height', img.attr('height')), img.attr('width') || $svg.removeAttr('width')),
              img.replaceWith($svg)
          })
        })
      }),
  ...
  (function (UI) {
    ...
    function scrollToElement (ele, options) {
      options = UI.$.extend({ duration: 1e3, transition: 'easeOutExpo', offset: 0, complete: function () {} }, options)
      var target = ele.offset().top - options.offset,
        docheight = UI.$doc.height(),
        winheight = window.innerHeight
      docheight < target + winheight && (target = docheight - winheight),
        UI.$('html,body')
          .stop()
    ...
    UI.component('smoothScroll', {
      boot: function () {
        UI.$html.on('click.smooth-scroll.uikit', '[data-uk-smooth-scroll]', function (e) {
          ...
          if (!ele.data('smoothScroll')) {
            UI.smoothScroll(ele, UI.Utils.options(ele.attr('data-uk-smooth-scroll')))
            ele.trigger('click')
          }
          return !1
        })
      ...
      init: function () {
        ...
        this.on('click', function (e) {
          e.preventDefault(), scrollToElement(UI.$(this.hash).length ? UI.$(this.hash) : UI.$('body'), $this.options)
        })
      }
    }),
  ...
  (function (UI) {
    ...
    var $win = UI.$win,
      ...
      checkScrollSpy = function () {
        for (var i = 0; i < scrollspies.length; i++) window.requestAnimationFrame.apply(window, [scrollspies[i].check])
      }
        ...
        $doc.on('scrolling.uk.document', checkScrollSpy),
          ...
          UI.ready(function (context) {
            UI.$('[data-uk-scrollspy]', context).each(function () {
              var element = UI.$(this)
              if (!element.data('scrollspy')) UI.scrollspy(element, UI.Utils.options(element.attr('data-uk-scrollspy')))
            })
          })
      },
      init: function () {
        var initinview,
          ...
          fn = function () {
            var elements = $this.options.target ? $this.element.find($this.options.target) : $this.element,
              delayIdx = 1 === elements.length ? 1 : 0,
              toggleclsIdx = 0
            elements.each(function (idx) {
              ...
              !inview ||
                ...
                element.data(
                  ...
                  setTimeout(function () {
                    element
                      ...
                      element.data('scrollspy-idle', !1),
                      element.data('inviewstate', !0)
                ...
                !inview &&
                  ...
                  $this.options.repeat &&
                  (element.data('scrollspy-idle') &&
                    (clearTimeout(element.data('scrollspy-idle')), element.data('scrollspy-idle', !1)),
                  element.removeClass('uk-scrollspy-inview').toggleClass(toggle),
                  element.data('inviewstate', !1),
                  element.trigger('outview.uk.scrollspy')),
                (toggleclsIdx = togglecls[toggleclsIdx + 1] ? toggleclsIdx + 1 : 0)
            })
          }
        fn(), (this.check = fn), scrollspies.push(this)
      }
    })
    var scrollspynavs = [],
          ...
          UI.ready(function (context) {
            UI.$('[data-uk-scrollspy-nav]', context).each(function () {
              ...
              if (!element.data('scrollspynav'))
                UI.scrollspynav(element, UI.Utils.options(element.attr('data-uk-scrollspy-nav')))
            })
          })
      },
      init: function () {
        var inviews,
          ...
          links = this.find("a[href^='#']").each(function () {
            '#' !== this.getAttribute('href').trim() && ids.push(this.getAttribute('href'))
          }),
          targets = UI.$(ids.join(',')),
          ...
          fn = function () {
            ...
            for (var i = 0; i < targets.length; i++)
              UI.Utils.isInView(targets.eq(i), $this.options) && inviews.push(targets.eq(i))
            if (inviews.length) {
              var navitems,
                scrollTop = $win.scrollTop(),
                target = (function () {
                  for (var i = 0; i < inviews.length; i++) if (inviews[i].offset().top >= scrollTop) return inviews[i]
                })()
              if (!target) return
              ;(navitems = $this.options.closest
                ? (links
                    .blur()
                    .closest(clsClosest)
    ...
    UI.component('toggle', {
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-toggle]', context).each(function () {
            var ele = UI.$(this)
            if (!ele.data('toggle')) UI.toggle(ele, UI.Utils.options(ele.attr('data-uk-toggle')))
          }),
            setTimeout(function () {
      ...
      init: function () {
        ...
        ;(this.aria = -1 !== this.options.cls.indexOf('uk-hidden')),
          this.getToggles(),
          this.on('click', function (e) {
            $this.element.is('a[href="#"]') && e.preventDefault(), $this.toggle()
          }),
          toggles.push(this)
      },
      toggle: function () {
        if (this.totoggle.length) {
          if (this.options.animation && UI.support.animation) {
            ...
            1 == animations.length && (animations[1] = animations[0]),
                ...
                ele.hasClass($this.options.cls)
                  ? (ele.toggleClass($this.options.cls),
                    UI.Utils.animate(ele, animations[0]).then(function () {
                      ele.css('animation-duration', ''), UI.Utils.checkDisplay(ele)
                    }))
                  : UI.Utils.animate(this, animations[1] + ' uk-animation-reverse').then(function () {
                      ele.toggleClass($this.options.cls).css('animation-duration', ''), UI.Utils.checkDisplay(ele)
                    })
              })
          } else this.totoggle.toggleClass(this.options.cls), UI.Utils.checkDisplay(this.totoggle)
          this.updateAria()
        }
      },
      getToggles: function () {
        ;(this.totoggle = this.options.target ? UI.$(this.options.target) : []), this.updateAria()
      },
      updateAria: function () {
        this.aria &&
      ...
      boot: function () {
        UI.$html.on('click.alert.uikit', '[data-uk-alert]', function (e) {
          ...
          if (!ele.data('alert')) {
            ...
            UI.$(e.target).is(alert.options.trigger) && (e.preventDefault(), alert.close())
          }
        })
      },
      init: function () {
      ...
      close: function () {
        ...
        this.options.fade
          ? element
              ...
              .animate(
                { height: 0, opacity: 0, 'padding-top': 0, 'padding-bottom': 0, 'margin-top': 0, 'margin-bottom': 0 },
        ...
        UI.$html.on('click.buttonradio.uikit', '[data-uk-button-radio]', function (e) {
          ...
          if (!ele.data('buttonRadio')) {
            var obj = UI.buttonRadio(ele, UI.Utils.options(ele.attr('data-uk-button-radio'))),
              target = UI.$(e.target)
            target.is(obj.options.target) && target.trigger('click')
          }
        })
      },
      init: function () {
      ...
      UI.component('button', {
        ...
        boot: function () {
          UI.$html.on('click.button.uikit', '[data-uk-button]', function (e) {
            var ele = UI.$(this)
            if (!ele.data('button')) {
              UI.button(ele, UI.Utils.options(ele.attr('data-uk-button')))
              ele.trigger('click')
            }
          })
        },
  ...
  (function (UI) {
    ...
    function justify (ele, justifyTo, boundarywidth, offset) {
      if (
        ...
        if ((ele.css('min-width', jwidth), 'right' == UI.langdirection)) {
          var right1 = boundarywidth - (justifyTo.offset().left + jwidth),
            right2 = boundarywidth - (ele.offset().left + ele.outerWidth())
          ele.css('margin-right', right1 - right2)
        } else ele.css('margin-left', justifyTo.offset().left - offset.left)
      }
    }
    UI.component('dropdown', {
      ...
      boot: function () {
        ...
        UI.$html.on(triggerevent + '.dropdown.uikit', '[data-uk-dropdown]', function (e) {
          ...
          if (!ele.data('dropdown')) {
            ...
            ;('click' == triggerevent || ('mouseenter' == triggerevent && 'hover' == dropdown.options.mode)) &&
              dropdown.element.trigger(triggerevent),
              dropdown.element.find(dropdown.options.dropdownSelector).length && e.preventDefault()
          }
        })
      },
      init: function () {
        ...
        ;(this.dropdown = this.find(this.options.dropdownSelector)),
          (this.offsetParent = this.dropdown
            .parents()
            .filter(function () {
              return -1 !== UI.$.inArray(UI.$(this).css('position'), ['relative', 'fixed', 'absolute'])
            })
            .slice(0, 1)),
          (this.centered = this.dropdown.hasClass('uk-dropdown-center')),
          (this.justified = !!this.options.justify && UI.$(this.options.justify)),
          (this.boundary = UI.$(this.options.boundary)),
          this.boundary.length || (this.boundary = UI.$win),
          ...
          'click' == this.options.mode || UI.support.touch
            ...
            : this.on('mouseenter', function (e) {
                $this.trigger('pointerenter.uk.dropdown', [$this]),
                  ...
                  hoverIdle && clearTimeout(hoverIdle),
                  (active && active == $this) ||
                    (hoverIdle =
                      active && active != $this
                        ? setTimeout(function () {
                        ...
                        : setTimeout($this.show.bind($this), $this.options.delay))
              })
                  ...
                  $this.remainIdle && clearTimeout($this.remainIdle),
                    active && active == $this
                      ? (!$this.dropdown.find(e.target).length ||
                          ...
                          $target.parents('.uk-dropdown-close').length) &&
                        $this.hide()
                      : (($target.is("a[href='#']") || $target.parent().is("a[href='#']")) && e.preventDefault(),
                        $this.show())
                })
      },
      show: function () {
        UI.$html.off('click.outer.dropdown'),
          active && active != this && active.hide(!0),
          hoverIdle && clearTimeout(hoverIdle),
          ...
          UI.Utils.checkDisplay(this.dropdown, !0),
          (active = this).registerOuterClick()
      },
      hide: function (force) {
          ...
          setTimeout(function () {
            UI.$html.on('click.outer.dropdown', function (e) {
              ...
              active != $this ||
                ...
                ($this.hide(!0), UI.$html.off('click.outer.dropdown'))
            })
          }, 10)
      },
      checkDimensions: function () {
        if (this.dropdown.length) {
          ...
          var pp,
            ...
            width = (dropdown.show().offset(), dropdown.outerWidth()),
            height = dropdown.outerHeight(),
            boundarywidth = this.boundary.width(),
            dpos = (this.boundary[0] !== window && this.boundary.offset() && this.boundary.offset(), this.options.pos),
            variants = {
          ...
          if (
            ...
            switch (this.checkBoundary(pos.left + css.left, pos.top + css.top, width, height, boundarywidth)) {
              case 'x':
                'x' !== this.options.preventflip && (fdpos = flips.x[dpos] || 'right-top')
                break
              case 'y':
                'y' !== this.options.preventflip && (fdpos = flips.y[dpos] || 'top-left')
                break
              case 'xy':
                this.options.preventflip || (fdpos = flips.xy[dpos] || 'right-bottom')
            }
            fdpos &&
              ((pp = fdpos.split('-')),
              (css = variants[fdpos] ? variants[fdpos] : variants['bottom-left']),
              this.checkBoundary(pos.left + css.left, pos.top + css.top, width, height, boundarywidth) &&
                ((pp = dpos.split('-')), (css = variants[dpos] ? variants[dpos] : variants['bottom-left'])))
          }
          boundarywidth < width && (dropdown.addClass('uk-dropdown-stack'), this.trigger('stack.uk.dropdown', [this])),
            dropdown
              .css(css)
              .css('display', '')
      ...
      checkBoundary: function (left, top, width, height, boundarywidth) {
        var axis = ''
        return (
          (left < 0 || left - UI.$win.scrollLeft() + width > boundarywidth) && (axis += 'x'),
          (top - UI.$win.scrollTop() < 0 || top - UI.$win.scrollTop() + height > window.innerHeight) && (axis += 'y'),
          axis
        )
      }
    }),
          ...
          ;(this.justified = !!this.options.justify && UI.$(this.options.justify)),
            ...
            this.on({
              ...
              'show.uk.dropdown': function (e, dropdown) {
                var h = $this.dropdown.dropdown.outerHeight(!0)
                  ...
                  $this.overlay
                    ...
                    .animate({ height: h }, $this.options.duration, function () {
                      $this.dropdown.dropdown.css('visibility', ''),
                        ...
                        UI.Utils.checkDisplay($this.dropdown.dropdown, !0)
                    }),
                  ($this.pointerleave = !1)
              },
            ...
            this.overlay.on({
              mouseenter: function () {
                $this.remainIdle && (clearTimeout($this.dropdown.remainIdle), clearTimeout($this.remainIdle))
              },
              mouseleave: function () {
                $this.pointerleave &&
                  active &&
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-grid-match]', context).each(function () {
            var grid = UI.$(this)
            grid.data('gridMatchHeight') || UI.gridMatchHeight(grid, UI.Utils.options(grid.attr('data-uk-grid-match')))
          })
        })
      },
      init: function () {
      ...
      match: function () {
        ...
        if (firstvisible.length)
          return (
            100 <=
              Math.ceil(
                (100 * parseFloat(firstvisible.css('width'))) / parseFloat(firstvisible.parent().css('width'))
        ...
        boot: function () {
          UI.ready(function (context) {
            UI.$('[data-uk-grid-margin]', context).each(function () {
              var grid = UI.$(this)
              grid.data('gridMargin') || UI.gridMargin(grid, UI.Utils.options(grid.attr('data-uk-grid-margin')))
            })
          })
        },
        init: function () {
  ...
  (function (UI) {
    ...
    UI.component('modal', {
      ...
      hasTransitioned: !0,
      init: function () {
        if ((body || (body = UI.$('body')), this.element.length)) {
          var $this = this
          ;(this.paddingdir = 'padding-' + ('left' == UI.langdirection ? 'right' : 'left')),
            ...
            this.on('click', '.uk-modal-close', function (e) {
              ...
              UI.$(e.target)[0] == $this.element[0] && $this.options.bgclose && $this.hide()
            })
        }
      },
      toggle: function () {
      ...
      show: function () {
        if (this.element.length) {
          ...
          if (!this.isActive())
            return (
              this.options.modal && active && active.hide(!0),
              this.element.removeClass('uk-open').show(),
              this.resize(),
              this.options.modal && (active = this),
              (this.active = !0),
              activeCount++,
              ...
              UI.Utils.checkDisplay(this.dialog, !0),
              this
            )
        }
      },
      ...
      updateScrollable: function () {
        ...
        if (scrollable.length) {
          ...
          var offset = Math.abs(parseInt(this.dialog.css('margin-top'), 10)),
            ...
            h = window.innerHeight - 2 * (offset < 20 ? 20 : offset) - dh
          return scrollable.css({ 'max-height': h < this.options.minScrollHeight ? '' : h, height: '' }), !0
        }
        return !1
      },
      _hide: function () {
        ;(this.active = !1),
          0 < activeCount ? activeCount-- : (activeCount = 0),
          this.element.hide().removeClass('uk-open'),
          this.element.attr('aria-hidden', 'true'),
          activeCount || ($html.removeClass('uk-modal-page'), body.css(this.paddingdir, '')),
          active === this && (active = !1),
      ...
      UI.component('modalTrigger', {
        boot: function () {
          UI.$html.on('click.modal.uikit', '[data-uk-modal]', function (e) {
            var ele = UI.$(this)
            ;(ele.is('a') && e.preventDefault(), ele.data('modalTrigger')) ||
              UI.modalTrigger(ele, UI.Utils.options(ele.attr('data-uk-modal'))).show()
          }),
            UI.$html.on('keydown.modal.uikit', function (e) {
              active && 27 === e.keyCode && active.options.keyboard && (e.preventDefault(), active.hide())
            }),
            UI.$win.on(
              'resize orientationchange',
              UI.Utils.debounce(function () {
        ...
        init: function () {
          ...
          ;(this.options = UI.$.extend(
            { target: !!$this.element.is('a') && $this.element.attr('href') },
            this.options
          )),
      ...
      (UI.modal.dialog = function (content, options) {
        ...
        return (
          modal.on('hide.uk.modal', function () {
            modal.persist && (modal.persist.appendTo(modal.persist.data('modalPersistParent')), (modal.persist = !1)),
              modal.element.remove()
          }),
          (function (content, modal) {
            if (!modal) return
            'object' == typeof content
              ? (content = content instanceof jQuery ? content : UI.$(content)).parent().length &&
                ((modal.persist = content), modal.persist.data('modalPersistParent', content.parent()))
              : (content =
                  'string' == typeof content || 'number' == typeof content
                    ? UI.$('<div></div>').html(content)
                    : UI.$('<div></div>').html('UIkit.modal Error: Unsupported data type: ' + typeof content))
      ...
      (UI.modal.alert = function (content, options) {
        options = UI.$.extend(!0, { bgclose: !1, keyboard: !1, modal: !1, labels: UI.modal.labels }, options)
      ...
      (UI.modal.confirm = function (content, onconfirm, oncancel) {
        var options = 1 < arguments.length && arguments[arguments.length - 1] ? arguments[arguments.length - 1] : {}
        ...
        return (
          modal.element.find('.js-modal-confirm, .js-modal-confirm-cancel').on('click', function () {
            UI.$(this).is('.js-modal-confirm') ? onconfirm() : oncancel(), modal.hide()
          }),
          modal.on('show.uk.modal', function () {
            setTimeout(function () {
      ...
      (UI.modal.prompt = function (text, value, onsubmit, options) {
        ...
        var modal = UI.modal.dialog(
          ...
          input = modal.element
            ...
            .val(value || '')
            .on('keyup', function (e) {
              13 == e.keyCode && modal.element.find('.js-modal-ok').trigger('click')
            })
        return (
          modal.element.find('.js-modal-ok').on('click', function () {
            !1 !== onsubmit(input.val()) && modal.hide()
          }),
          modal.on('show.uk.modal', function () {
            setTimeout(function () {
              input.focus()
      ...
      (UI.modal.blockUI = function (content, options) {
        var modal = UI.modal.dialog(
          ...
          UI.$.extend({ bgclose: !1, keyboard: !1, modal: !1 }, options)
        )
        return (modal.content = modal.element.find('.uk-modal-content:first')), modal.show()
      }),
    ...
    UI.component('nav', {
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-nav]', context).each(function () {
            var nav = UI.$(this)
            if (!nav.data('nav')) UI.nav(nav, UI.Utils.options(nav.attr('data-uk-nav')))
          })
        })
      },
      init: function () {
      ...
      open: function (li, noanimation) {
        ...
        this.options.multiple ||
          ...
          $container &&
            ($li.hasClass('uk-open') && $container.removeClass('uk-hidden'),
            noanimation
              ? ($container.stop().height($li.hasClass('uk-open') ? 'auto' : 0),
                $li.hasClass('uk-open') || $container.addClass('uk-hidden'),
              ...
              : $container.stop().animate(
                  {
                    height: $li.hasClass('uk-open')
                      ? (function (ele) {
                          var $ele = UI.$(ele),
                            height = 'auto'
                          if ($ele.is(':visible')) height = $ele.outerHeight()
                          else {
                            var tmp = {
                  ...
                  function () {
                    $li.hasClass('uk-open') ? $container.css('height', '') : $container.addClass('uk-hidden'),
                      $this.trigger('display.uk.check')
                  }
                ))
      }
  ...
  (function (UI) {
    ...
    var scrollpos = { x: window.scrollX, y: window.scrollY },
      $html = (UI.$win, UI.$doc, UI.$html),
      Offcanvas = {
        show: function (element, options) {
          if ((element = UI.$(element)).length) {
            options = UI.$.extend({ mode: 'push' }, options)
            var $body = UI.$('body'),
              bar = element.find('.uk-offcanvas-bar:first'),
              rtl = 'right' == UI.langdirection,
            ...
            ;(scrollpos = { x: window.pageXOffset, y: window.pageYOffset }),
              ...
              ('push' != options.mode && 'reveal' != options.mode) ||
                $body.css(rtl ? 'margin-right' : 'margin-left', (rtl ? -1 : 1) * (bar.outerWidth() * dir)),
              'reveal' == options.mode && bar.css('clip', 'rect(0, ' + bar.outerWidth() + 'px, 100vh, 0)'),
              $html.css('margin-top', -1 * scrollpos.y).width(),
          ...
          panel.length &&
            ...
            UI.support.transition && !force
              ? ($body
                ...
                'reveal' == bar.attr('mode') && bar.css('clip', ''),
                setTimeout(function () {
                  bar.removeClass('uk-offcanvas-bar-show')
                }, 0))
              : finalize())
        ...
        _initElement: function (element) {
          element.data('OffcanvasInit') ||
            (element.on('click.uk.offcanvas swipeRight.uk.offcanvas swipeLeft.uk.offcanvas', function (e) {
              var target = UI.$(e.target)
              if (!e.type.match(/swipe/) && !target.hasClass('uk-offcanvas-close')) {
                if (target.hasClass('uk-offcanvas-bar')) return
                if (target.parents('.uk-offcanvas-bar:first').length) return
              }
              e.stopImmediatePropagation(), Offcanvas.hide()
            }),
            element.on('click', 'a[href*="#"]', function (e) {
              ...
              '#' != href &&
                (UI.$doc.one('hide.uk.offcanvas', function () {
                  ...
                  target.length || (target = UI.$('[name="' + link[0].hash.replace('#', '') + '"]')),
      ...
      boot: function () {
        $html.on('click.offcanvas.uikit', '[data-uk-offcanvas]', function (e) {
          ...
          if (!ele.data('offcanvasTrigger')) {
            UI.offcanvasTrigger(ele, UI.Utils.options(ele.attr('data-uk-offcanvas')))
            ele.trigger('click')
          }
        }),
          $html.on('keydown.uk.offcanvas', function (e) {
            27 === e.keyCode && Offcanvas.hide()
          })
      },
      init: function () {
        var $this = this
        ;(this.options = UI.$.extend(
          { target: !!$this.element.is('a') && $this.element.attr('href'), mode: 'push' },
          this.options
        )),
          this.on('click', function (e) {
            e.preventDefault(), Offcanvas.show($this.options.target, $this.options)
          })
      }
    }),
  ...
  (function (UI) {
    ...
    function coreAnimation (cls, current, next) {
      ...
      return (
        next[0] === current[0]
          ? d.resolve()
          : ('object' == typeof cls && ((clsIn = cls[0]), (clsOut = cls[1] || cls[0])),
            UI.$body.css('overflow-x', 'hidden'),
            (release = function () {
              current && current.hide().removeClass('uk-active ' + clsOut + ' uk-animation-reverse'),
                next
                  .addClass(clsIn)
                  .one(
                    ...
                    function () {
                      next.removeClass('' + clsIn).css({ opacity: '', display: '' }),
                        ...
                        current && current.css({ opacity: '', display: '' })
                    }.bind(this)
                  )
                  .show()
            }),
            ...
            current && current.length
              ? (current.css('animation-duration', this.options.duration + 'ms'),
                current
                  ...
                  .css('display', ''))
              : (next.addClass('uk-active'), release())),
        d.promise()
      )
    }
    UI.component('switcher', {
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-switcher]', context).each(function () {
            ...
            if (!switcher.data('switcher')) UI.switcher(switcher, UI.Utils.options(switcher.attr('data-uk-switcher')))
          })
        })
      },
      init: function () {
        ...
        if (
          (this.on('click.uk.switcher', this.options.toggle, function (e) {
            e.preventDefault(), $this.show(this)
          }),
            ...
            this.connect.length &&
              ...
              this.connect.on('click', '[data-uk-switcher-item]', function (e) {
                ...
                if ($this.index != item)
                  switch (item) {
                    case 'next':
                    case 'previous':
                      $this.show($this.index + ('next' == item ? 1 : -1))
                      break
                    default:
              ...
              this.options.swiping &&
                this.connect.on('swipeRight swipeLeft', function (e) {
                  e.preventDefault(),
                    window.getSelection().toString() || $this.show($this.index + ('swipeLeft' == e.type ? 1 : -1))
                }))
          var toggles = this.find(this.options.toggle),
            active = toggles.filter('.uk-active')
          if (active.length) this.show(active, !1)
          else {
            if (!1 === this.options.active) return
            ;(active = toggles.eq(this.options.active)), this.show(active.length ? active : toggles.eq(0), !1)
          }
      ...
      show: function (tab, animate) {
        if (!this.animating) {
          if (isNaN(tab)) tab = UI.$(tab)
          else {
            var toggles = this.find(this.options.toggle)
            ;(tab = tab < 0 ? toggles.length - 1 : tab), (tab = toggles.eq(toggles[tab] ? tab : 0))
          }
          var $this = this,
            active = ((toggles = this.find(this.options.toggle)), UI.$(tab)),
            animation =
              Animations[this.options.animation] ||
              function (current, next) {
                if (!$this.options.animation) return Animations.none.apply($this)
                var anim = $this.options.animation.split(',')
                return (
                  1 == anim.length && (anim[1] = anim[0]),
                  (anim[0] = anim[0].trim()),
                  (anim[1] = anim[1].trim()),
                  coreAnimation.apply($this, [anim, current, next])
                )
              }
          ;(!1 !== animate && UI.support.animation) || (animation = Animations.none),
            active.hasClass('uk-disabled') ||
      ...
      (Animations = {
        ...
        'slide-vertical': function (current, next, dir) {
          ...
          return (
            current && current.index() > next.index() && anim.reverse(),
            coreAnimation.apply(this, [anim, current, next])
          )
        },
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-tab]', context).each(function () {
            var tab = UI.$(this)
            if (!tab.data('tab')) UI.tab(tab, UI.Utils.options(tab.attr('data-uk-tab')))
          })
        })
      },
      init: function () {
        ...
        ;(this.current = !1),
          this.on('click.uk.tab', this.options.target, function (e) {
            if ((e.preventDefault(), !$this.switcher || !$this.switcher.animating)) {
              var current = $this.find($this.options.target).not(this)
              current.removeClass('uk-active').blur(),
                $this.trigger('change.uk.tab', [UI.$(this).addClass('uk-active'), $this.current]),
                ($this.current = UI.$(this)),
                $this.options.connect ||
                  (current.attr('aria-expanded', 'false'), UI.$(this).attr('aria-expanded', 'true'))
            }
          }),
          this.options.connect && (this.connect = UI.$(this.options.connect)),
      ...
      boot: function () {
        UI.ready(function (context) {
          UI.$('[data-uk-cover]', context).each(function () {
            var ele = UI.$(this)
            if (!ele.data('cover')) UI.cover(ele, UI.Utils.options(ele.attr('data-uk-cover')))
          })
        })
      },
      init: function () {
        if (
          ...
          this.element
            ...
            .attr('src', [src, -1 < src.indexOf('?') ? '&' : '?', 'enablejsapi=1&api=1'].join(''))
        }
      },
      check: function () {
        ...
        ;(height =
          w / this.ratio < h ? ((width = Math.ceil(h * this.ratio)), h) : ((width = w), Math.ceil(w / this.ratio))),
          ...
          UI.ready(function (context) {
            setTimeout(function () {
              UI.$('[data-uk-accordion]', context).each(function () {
                var ele = UI.$(this)
                ele.data('accordion') || UI.accordion(ele, UI.Utils.options(ele.attr('data-uk-accordion')))
              })
            }, 0)
          })
        },
            ...
            animated
              ? wrapper.animate(
                  ...
                  },
                  {
                    ...
                    complete: function () {
                      active &&
                        (wrapper.css({ overflow: '', height: 'auto' }), UI.Utils.checkDisplay(wrapper.data('content'))),
                        $this.trigger('display.uk.check')
                    }
                  }
                )
              : (wrapper.height(active ? 'auto' : 0),
                active && (wrapper.css({ overflow: '' }), UI.Utils.checkDisplay(wrapper.data('content'))),
                this.trigger('display.uk.check')),
        ...
        update: function () {
          ...
          ;(this.toggle = this.find(this.options.toggle)),
            ...
            this.content.each(function (index) {
              ;($content = UI.$(this)).parent().data('wrapper')
      ...
      UI.component('autocomplete', {
        ...
        init: function () {
          ...
          ;(this.dropdown = this.find('.uk-dropdown')),
            ...
            this.dropdown.length || (this.dropdown = UI.$('<div class="uk-dropdown"></div>').appendTo(this.element)),
              ...
              case 'object':
                if (source.length) {
                  ...
                  source.forEach(function (item) {
                    item.value && -1 != item.value.toLowerCase().indexOf($this.value.toLowerCase()) && items.push(item)
                  }),
                    release(items)
                }
                break
      ...
      UI.component('datepicker', {
        defaults: {
          ...
          template: function (data, opts) {
            ...
            if (
              ...
              content +=
                '<div class="uk-datepicker-heading">' + opts.i18n.months[data.month] + ' ' + data.year + '</div>'
            for (
              content += '</div>', content += '<table class="uk-datepicker-table">', content += '<thead>', i = 0;
              i < data.weekdays.length;
              i++
            )
              data.weekdays[i] && (content += '<th>' + data.weekdays[i] + '</th>')
            for (content += '</thead>', content += '<tbody>', i = 0; i < data.days.length; i++)
            ...
            return (content += '</tbody>'), (content += '</table>')
          }
        },
        boot: function () {
        ...
        init: function () {
          if (!UI.support.touch || 'date' != this.element.attr('type') || this.options.mobile) {
            ...
            ;(this.current = this.element.val() ? moment(this.element.val(), this.options.format) : moment()),
              ...
              dropdown ||
                ((dropdown = UI.$('<div class="uk-dropdown uk-datepicker"></div>')).on(
                  'click',
        ...
        getRows: function (year, month) {
          var opts = this.options,
            ...
            data = { month: month, year: year, weekdays: [], days: [], maxDate: !1, minDate: !1 },
            row = []
          !1 !== opts.maxDate &&
          ...
          for (var day, isDisabled, isSelected, isToday, isInMonth, cells = days + before, after = cells; 7 < after; )
            after -= 7
          cells += 7 - after
          ...
          ;(this.target = this.find(this.options.target)),
            ...
            this.select.on(
              ...
              (fn = function () {
                ...
                return $this.element[$this.select.val() ? 'addClass' : 'removeClass']($this.options.activeClass), fn
              })())
            ),
      ...
      UI.component('htmleditor', {
        ...
        boot: function () {
          UI.ready(function (context) {
            UI.$('textarea[data-uk-htmleditor]', context).each(function () {
              var editor = UI.$(this)
              editor.data('htmleditor') || UI.htmleditor(editor, UI.Utils.options(editor.attr('data-uk-htmleditor')))
            })
          })
        },
        init: function () {
            ...
            this.options.autocomplete &&
              ...
              this.editor.on(
                ...
                UI.Utils.debounce(function () {
                  ...
                  if (
                    'xml' ==
                    $this.CodeMirror.innerMode($this.editor.getMode(), $this.editor.getTokenAt(POS).state).mode.name
                  ) {
        ...
        replaceSelection: function (replace) {
          ...
          if (!text.length) {
            ...
            curWord &&
              (this.editor.setSelection({ line: cur.line, ch: start }, { line: cur.line, ch: end }), (text = curWord))
          }
            ...
            editor.on('action.listOl', function () {
              if ('markdown' == editor.getCursorMode()) {
                var matches,
                  ...
                  prefix = 1
                if (0 < pos.line)
                  (matches = cm.getLine(pos.line - 1).match(/^(\d+)\./)) && (prefix = Number(matches[1]) + 1)
      ...
      UI.component('lightbox', {
        ...
        show: function (index) {
          ;(this.modal = (function (lightbox) {
            if (modal) return (modal.lightbox = lightbox), modal
            return (
              ((modal = UI.$(
                [
                  '<div class="uk-modal">',
              ...
              modal
                ...
                .on('click', '[data-lightbox-previous], [data-lightbox-next]', function (e) {
                  e.preventDefault(), modal.lightbox[UI.$(this).is('[data-lightbox-next]') ? 'next' : 'previous']()
                }),
                ...
                ;(img.onerror = function () {
                  ...
                  (img.onload = function () {
                    120 == img.width && 90 == img.height
                      ...
                      : ((cache[id] = { width: img.width, height: img.height }), resolve(id, img.width, img.height))
                  }),
      ...
      (UI.lightbox.create = function (items, options) {
        if (items) {
          ...
          return (
            ...
            UI.lightbox(UI.$.extend({}, options, { group: group }))
          )
        }
      }),
      UI.lightbox
    ...
    return (
      UI.component('nestable', {
          ...
          var onStartEvent = function (e) {
            ...
            onMoveEvent = function (e) {
              e.originalEvent && e.originalEvent.touches && (e = e.originalEvent.touches[0]),
                ...
                $this.dragEl && (e.preventDefault(), $this.dragMove(e), $this.trigger('move.uk.nestable', [$this]))
            },
          ...
          var data = [],
            step = function (level, depth, parent) {
              level.children(options._listItemClass).each(function (index) {
                var li = UI.$(this),
                  item = UI.$.extend({ parent_id: parent || null, depth: depth, order: index }, li.data()),
        ...
        unsetParent: function (li) {
          li.removeClass('uk-parent ' + this.options.collapsedClass), li.children(this.options._listClass).remove()
        },
          ...
          if (
            ...
            mouse.dirAx &&
              ...
              0 < mouse.distX &&
                ...
                this.placeEl.parents(opt._listClass + ',' + opt._listBaseClass).length + this.dragDepth <= maxDepth &&
                  (list.length
                    ? (list = prev.children(opt._listClass).last()).append(this.placeEl)
                    : ((list = UI.$('<ul/>').addClass(opt.listClass)).append(this.placeEl),
                      prev.append(list),
                      this.setParent(prev)))),
          ...
          if (!this.placeEl.find(this.pointEl).length) {
            ...
            if (!mouse.dirAx || isNewRoot || isEmpty) {
              ...
              if (
                ...
                maxDepth < this.dragDepth - 1 + this.pointEl.parents(opt._listClass + ',' + opt._listBaseClass).length)
              )
                return
              ...
              ;(parent = this.placeEl.parent()),
                ...
                isNewRoot && ((this.dragRootEl = tmpRoot), (this.hasNewRoot = this.element[0] !== this.dragRootEl[0]))
            }
          }
        },
  ...
  (function (addon) {
    ...
    UI.component('parallax', {
        ...
        ;(supports3d = (function () {
          ...
          UI.ready(function (context) {
            UI.$('[data-uk-parallax]', context).each(function () {
              ...
              parallax.data('parallax') || UI.parallax(parallax, UI.Utils.options(parallax.attr('data-uk-parallax')))
            })
          })
      },
      init: function () {
      ...
      process: function () {
        ...
        !1 !== this.options.viewport && (percent = 0 === this.options.viewport ? 1 : percent / this.options.viewport),
          this.update(percent)
      },
        ...
        updateFocus: function (idx, dir) {
          if (this.active) {
            ...
            this.items.removeClass('uk-slide-before uk-slide-after').each(function (i) {
              i !== idx && UI.$(this).addClass(UI.$(this).data('left') < left ? 'uk-slide-before' : 'uk-slide-after')
            }),
              (this.focus = idx),
      ...
      UI.$doc.on('mousemove.uk.slider touchmove.uk.slider', function (e) {
        if (
          ...
          if (
            (e.clientX || e.clientY
              ? (x = e.clientX)
              : (e.pageX || e.pageY) && (x = e.pageX - document.body.scrollLeft - document.documentElement.scrollLeft),
            (focus = store.focus),
            ...
            for (diff = item.data('left') + Math.abs(xDiff), i = 0, z = store.focus; i < dragging.items.length; i++) {
              if (
                ((itm = dragging.items.eq(z)), z != store.focus && itm.data('left') < diff && itm.data('area') > diff)
              ) {
                focus = z
                break
              }
        ...
        ;(this.container = this.element.hasClass('uk-slideshow') ? this.element : UI.$(this.find('.uk-slideshow'))),
          ...
          UI.$win.on(
            ...
            UI.Utils.debounce(function () {
              $this.resize(),
                $this.fixFullscreen &&
                  ($this.container.css('height', window.innerHeight), $this.slides.css('height', window.innerHeight))
            }, 100)
          ),
          setTimeout(function () {
            $this.resize()
        ...
        scroll: function (current, next, dir) {
          ...
          return (
            ...
            next.css('opacity', 1).one(
              UI.support.animation.end,
              function () {
                current.removeClass(-1 == dir ? 'uk-slideshow-scroll-backward-out' : 'uk-slideshow-scroll-forward-out'),
                  next
                    .css('opacity', '')
                    .removeClass(-1 == dir ? 'uk-slideshow-scroll-backward-in' : 'uk-slideshow-scroll-forward-in'),
                  d.resolve()
              }.bind(this)
            ),
        ...
        swipe: function (current, next, dir) {
          ...
          return (
            ...
            next.css('opacity', 1).one(
              UI.support.animation.end,
              function () {
                current.removeClass(-1 === dir ? 'uk-slideshow-swipe-backward-out' : 'uk-slideshow-swipe-forward-out'),
                  next
                    .css('opacity', '')
    ...
    UI.$.extend(UI.slideshow.animations, {
      slice: function (current, next, dir, fromfx) {
        ...
        for (
          ...
          'slice-up-down' == fromfx && (((i % 2) + 2) % 2 == 0 ? '0' : ghostHeight)
          var clipfrom,
      ...
      fold: function (current, next, dir) {
        ...
        for (
          var bar,
            ...
            sliceWidth = Math.ceil(this.element.width() / this.options.slices),
            bgimage = next.data('cover').css('background-image'),
            ghost = UI.$('<li></li>').css({ width: next.width(), height: next.height(), opacity: 1, zIndex: 15 }),
          ...
          ;(this.wrapper = this.element.wrap('<div class="uk-sticky-placeholder"></div>').parent()),
            ...
            boundary &&
              (!0 === boundary || '!' === boundary[0]
                ? ((boundary = !0 === boundary ? this.wrapper.parent() : this.wrapper.closest(boundary.substr(1))),
                  (boundtoparent = !0))
      ...
      init: function () {
          ...
          times = (function (start, end) {
            ...
            for (i = start = start || 0, h = ''; i < end; i++)
              (h = '' + i),
                ...
                0 === i &&
                  ((h = 12), times['12h'].push({ value: h + ':00 AM' }), times['12h'].push({ value: h + ':30 AM' })),
                0 < i &&
                  i < 13 &&
                  12 !== i &&
        ...
        ;(this.options.minLength = 0),
          ...
          (container = this.element.is('input')
            ? (this.element.wrap('<div class="uk-autocomplete"></div>'), this.element.parent())
      ...
      UI.component('tooltip', {
        defaults: {
          ...
          src: function (ele) {
            ...
            return void 0 !== title && ele.data('cached-title', title).removeAttr('title'), ele.data('cached-title')
          }
        },
        tip: '',
        boot: function () {
        ...
        init: function () {
          var $this = this
          $tooltip || ($tooltip = UI.$('<div class="uk-tooltip"></div>').appendTo('body')),
            this.on({
              focus: function (e) {
                $this.show()
              },
        ...
        show: function () {
          if (
            ...
            var $this = this,
              ...
              position = 'function' == typeof this.options.pos ? this.options.pos.call(this.element) : this.options.pos,
              tmppos = position.split('-'),
              tcss = { display: 'none', visibility: 'visible', top: pos.top + pos.height + height, left: pos.left }
          ...
          settings.beforeAll(files),
            (settings.complete = function (response, xhr) {
              ;(uploaded += 1),
                ...
                allow && uploaded < count ? upload([files[uploaded]], settings) : settings.allcomplete(response, xhr)
            }),
    ...
    function matchName (pattern, path) {
      var parsedPattern =
        ...
        pattern
          ...
          .replace(/((?!\\))\?/g, '$1.') +
        '$'
      return (parsedPattern = '^' + parsedPattern + '$'), null !== path.match(new RegExp(parsedPattern, 'i'))
    }
    return (
