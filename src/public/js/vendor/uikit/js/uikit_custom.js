...
(function(UI) {
    ...
    UI.modal.confirm = function(content, onconfirm, oncancel) {
        ...
        options   = UI.$.extend(true, {bgclose:false, keyboard:false, modal:false, labels:UI.modal.labels, confirmButtonClass: ''}, UI.$.isFunction(options) ? {}:options);

        var modal = UI.modal.dialog(([
            '<div class="uk-margin uk-modal-content">'+String(content)+'</div>',
            '<div class="uk-modal-footer uk-text-right"><button class="uk-button js-modal-confirm-cancel">'+options.labels.Cancel+'</button> <button class="uk-button uk-button-primary js-modal-confirm ' + options.confirmButtonClass + '">'+options.labels.Ok+'</button></div>'
        ]).join(""), options);
