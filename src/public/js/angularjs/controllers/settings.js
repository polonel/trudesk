  ...
  return angular
    ...
    .controller('settingsCtrl', function (SettingsService, $scope, $http, $timeout, $log, $window) {
      ...
      $scope.switchSettings = function (event, settings) {
        if (settings) {
          ...
          if ($target.length > 0) {
            ...
            if (settings === 'settings-legal' && privacyPolicyMDE) privacyPolicyMDE.codemirror.refresh()
          }
        }
      }
      ...
      $scope.showCreateTicketTypeWindow = function ($event) {
        ...
        if (createTicketTypeModal.length > 0) UIkit.modal(createTicketTypeModal, { bgclose: false }).show()
      }
