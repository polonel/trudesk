...
define(['angular', 'underscore', 'jquery', 'modules/helpers', 'uikit'], function (angular, _, $, helpers, UIkit) {
  return angular
    .module('trudesk.controllers.settings.permissions', ['ngSanitize'])
    .controller('PermissionsCtrl', function ($scope, $http, $timeout, $log) {
        ...
        $timeout(function () {
          ...
          if (permissionList.length > 0) {
            ...
            permissionSort.on('change.uk.sortable', function () {
              ...
              var arr = []
              for (var k = 0; k < items.length; k++) arr.push($(items[k]).attr('data-key'))

              $http
                .put(
                  '/api/v1/settings/updateroleorder',
                  {
                    roleOrder: arr
                  },
                  {
          ...
          if ($ticketsAllCheck.is(':checked')) toggleAll($ticketsAllCheck.parents('form'), 'tickets', true)
          ...
          $('input[name="perm-accounts-import"]').change(function (e) {
            ...
            if (checked) $('input[name="perm-accounts-create"]').prop('checked', checked)
          })
        }, 0)
      }
      ...
      function toggleAll ($form, type, checked, specials) {
        $form
          .find('input[name="perm-' + type + '-create"]')
          .prop('checked', checked)
          .prop('disabled', checked)
        $form
          .find('input[name="perm-' + type + '-view"]')
          .prop('checked', checked)
          .prop('disabled', checked)
        $form
          .find('input[name="perm-' + type + '-edit"]')
          .prop('checked', checked)
          .prop('disabled', checked)
        $form
          .find('input[name="perm-' + type + '-delete"]')
          .prop('checked', checked)
          .prop('disabled', checked)
        ...
        if (specials && specials.length > 0) {
          for (var i = 0; i < specials.length; i++)
            $form
              .find('input[name="perm-' + type + '-' + specials[i] + '"]')
              .prop('checked', checked)
              .prop('disabled', checked)
        }
      }
      ...
      function sanitizePermissions (data) {
        ...
        // Agent
        if (tObj.isagent) obj.agent = ['*']

        // Tickets
        if (tObj.ticketsall) obj.ticket = ['*']
        else {
          ...
          if (arr.length > 0) obj.ticket = arr
        }

        // Accounts
        if (tObj.accountsall) obj.account = ['*']
        else {
          ...
          if (arr.length > 0) obj.account = arr
        }
