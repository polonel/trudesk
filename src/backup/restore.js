...
function runRestore (file, callback) {
  ...
  var options = [
    ...
    '-d',
    databaseName,
    path.join(__dirname, '../../restores/restore_' + file, 'database/trudesk')
  ]
  var mongodump = null
  if (platform === 'win32') mongodump = spawn(path.join(__dirname, 'bin', platform, 'mongorestore'), options)
  else mongodump = spawn('mongorestore', options)
