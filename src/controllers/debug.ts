...
const debugController: Record<string, any> = {}

debugController.content = {}

debugController.populatedatabase = function (_req: any, res: any) {
  ...
  async.series(
    [
      ...
      function (done: any) {
        ...
        for (let i = 0; i < 11; i++) {
          ...
          const group = {
            name: name,
            __v: 0,
            members: _.map(users, function (o: any) {
              return o._id
            })
          }
      ...
      function (done: any) {
        userSchema.find({}).then(function (users: any[]) {

          groupSchema.getAllGroups(function (err: any, groups: any[]) {
            if (err) return done(err)

            ticketTypeSchema.getTypes(function (err: any, types: any[]) {
              if (err) return done(err)

              tagSchema.getTags(function (err: any, tags: any[]) {
                if (err) return done(err)
...
debugController.sendmail = function (req: any, res: any) {
  ...
  email
    .render('ticket-comment-added', { base_url: (global as any).TRUDESK_BASEURL, ticket: ticket, comment: ticket.comments[0] })
...
debugController.uploadPlugin = function (req: any, res: any) {
  const fs = require('fs')
  ...
  busboy.on('file', function (_fieldname: string, file: any, filename: string, _encoding: string, mimetype: string) {
    winston.debug('uploadPlugin mimetype: ' + mimetype)
    ...
    winston.debug('uploadPlugin object: ' + JSON.stringify(object))
