...
findGroup = async function (group, res) {
  return await new Promise(function (resolve, reject) {
    Group.findOne({ domainName: group.domainName }, function (err, groupDB) {
      if (err || (groupDB !== null && groupDB?.name !== group?.name && groupDB?.domainName !== ''))
        return apiUtils.sendApiError(res, 400, 'The domain is already used by the group ' + group.name);
      resolve(true);
    });
  });
};
...
addDomain = function (group, res) {
  if (group == undefined || group.domainName == undefined)
    return apiUtils.sendApiError(res, 400, 'Invalid Domain or Group Name');

  Domain.findOne({ name: group.domainName }, function (err, domain) {
    ...
    if (domain !== null) {
      ...
      Domain.insertMany({ name: group.domainName, groupID: group._id }, function (err, domains) {
        //Если домена нет то добавить его в базу
        let domain = null;
        if (domains) {
          domain = domains[0];
        }
        if (domain !== null) {
          //Если домен добавлен в базу
          if (domain._id !== group.domainID || group.domainID == undefined) {
            //Если ID домена группы неопределён или не равен новому
            group.domainID = domain._id; //Привязать группе ID домена
            group.save(function (err, group) {
              //Сохранить группу
              if (err) return apiUtils.sendApiError(res, 500, err.message);
