...
const apiTags: Record<string, any> = {}

apiTags.create = async (req: any, res: any) => {
  const data = req.body
...
apiTags.getTagsWithLimit = async (req: any, res: any) => {
  const qs = req.query
...
apiTags.updateTag = async (req: any, res: any) => {
...
apiTags.deleteTag = async (req: any, res: any) => {
  const id = req.params.id
  ...
  try {
    ...
    for (const ticket of tickets) {
      ticket.tags = reject(ticket.tags, (o: any) => o._id.toString() === id.toString())
      await ticket.save()
    }
