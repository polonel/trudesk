...
const groupsController: Record<string, any> = {}
...
groupsController.get = async (req: any, res: any) => {
  const user = req.user
...
groupsController.getCreate = async function (req: any, res: any) {
  const user = req.user
...
groupsController.edit = function (req: any, res: any) {
  const user = req.user
  ...
  try {
    ...
    const [users, group] = Promise.all([getUsers, getGroup]) as any
