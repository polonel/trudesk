...
mainController.about = function (req, res) {
  ...
  settings.getSettingByName('legal:privacypolicy', function (err, privacyPolicy) {
    if (err) return res.render('error', { layout: false, error: err })
    var content = {}
    content.title = 'About'
    content.nav = 'about'
    ...
    else content.data.privacyPolicy = marked(privacyPolicy.value)
...
mainController.loginPost = function (req, res, next) {
  passport.authenticate('local', function (err, user) {
    ...
    if (req.user.role === 'user') redirectUrl = '/tickets'
...
mainController.forgotL2Auth = function (req, res) {
  var data = req.body
  if (_.isUndefined(data['forgotl2auth-email'])) return res.status(400).send('No Form Data')
  ...
  userSchema.getUserByEmail(email, function (err, user) {
    ...
    if (!user) return res.status(400).send('Invalid Email: Account not found!')
...
mainController.forgotPass = function (req, res) {
  var data = req.body
  if (_.isUndefined(data['forgotPass-email'])) return res.status(400).send('No Form Data')
...
mainController.resetl2auth = function (req, res) {
  ...
  if (_.isUndefined(hash)) return res.status(400).send('Invalid Link!')

  var userSchema = require('../models/user')
  userSchema.getUserByL2ResetHash(hash, function (err, user) {
    if (err) return res.status(400).send('Invalid Link!')

    if (_.isUndefined(user) || _.isEmpty(user)) return res.status(400).send('Invalid Link!')
    ...
    } else return res.status(400).send('Invalid Link!')
  })
}
...
mainController.l2authget = function (req, res) {
  ...
  settings.getSettingByName('mailer:enable', function (err, setting) {
    ...
    if (!_.isNull(setting)) content.mailerEnabled = setting.value
