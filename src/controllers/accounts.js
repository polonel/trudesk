...
accountsController.uploadCSV = function(req, res) {
    ...
    .on('end', function() {
        ...
        var usernameIdx = _.findIndex(titleRow, function(i) { return i.toLowerCase() === 'username'; });
        var fullnameIdx = _.findIndex(titleRow, function(i) { return i.toLowerCase() === 'fullname'; });
        var emailIdx = _.findIndex(titleRow, function(i) { return i.toLowerCase() === 'email'; });
        var titleIdx = _.findIndex(titleRow, function(i) { return i.toLowerCase() === 'title'; });
        var roleIdx = _.findIndex(titleRow, function(i) { return i.toLowerCase() === 'role'; });
    ...
    busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
        ...
        file.on('end', function() {
            ...
            async.eachSeries(accounts, function(item, next) {
                ...
                return res.json({success: true, contents: object.json, addedUsers: addedUsers, updatedUsers: updatedUsers});
            });
        }).setEncoding('utf8');
    });
